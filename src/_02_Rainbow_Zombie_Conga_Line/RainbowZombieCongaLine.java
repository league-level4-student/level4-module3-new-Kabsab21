...
public class RainbowZombieCongaLine {
	...
	public void everyoneOut(Zombie dancer) {
		...
		for (int i = 0; i < congaLine.size(); i++) {
			if (base.getValue().getZombieHatColor() == dancer.getZombieHatColor()) {
				congaLine.remove(i);
			}
	...
	public void youAreDone(Zombie dancer) {
		...
		for (int i = 0; i < congaLine.size(); i++) {
			...
			if (base.getValue().getZombieHatColor().equals(zombie.getValue().getZombieHatColor())) {
	...
	public void brains(Zombie dancer) {
		congaLine.add(new Zombie(dancer.getZombieHatColor()));
		congaLine.add(new Zombie(dancer.getZombieHatColor()));
	}
	...
	public void rainbowBrains(Zombie dancer) {
		Node base = congaLine.getHead();
		engine(dancer);
		for (int i = 0; i <= 6; i++) {
			Node i = congaLine.add(new Zombie(zombieHats[i]));
			base.setNext( )
		}
	}
